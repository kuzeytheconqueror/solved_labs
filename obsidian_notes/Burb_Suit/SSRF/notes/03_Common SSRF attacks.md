...
> [!example] Lab
> [Basic SSRF against the local server](https://portswigger.net/web-security/ssrf/lab-basic-ssrf-against-localhost) — notes at [[Basic SSRF against the local server]].
...
In some cases, the application server is able to interact with back-end systems that are not directly reachable by users. These systems often have non-routable private IP addresses and are protected only by the network topology, so they frequently have a weaker security posture — sensitive functionality may be reachable without authentication by anyone who can talk to them.
