...
> [!example] Lab
> [Indirect prompt injection via AI-powered scan](https://portswigger.net/web-security/llm-attacks/ai-powered-scanner-vulnerabilities/lab-indirect-prompt-injection-via-ai-powered-scan) — notes at [[Exploiting AI agents to perform destructive actions]].
...
> [!example] Lab
> [Sensitive information exfiltration](https://portswigger.net/web-security/llm-attacks/ai-powered-scanner-vulnerabilities/lab-sensitive-information-exfiltration) — notes at [[Exploiting AI agents to exfiltrate sensitive information]].
...
1. The attacker injects a prompt instructing the scanner to send a request to an internal path, such as `/admin`, with a modified Host header pointing to an internal IP address.
2. The scanner executes the request from its privileged position inside the internal network.
3. The response is processed by the scanner and can be exfiltrated back to the attacker, for example by posting it as a comment.
...
>For further information on exploiting routing-based SSRF, see the [Routing-based SSRF lab](https://portswigger.net/web-security/host-header/exploiting/lab-host-header-routing-based-ssrf).
