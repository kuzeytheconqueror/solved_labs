...
You can use an injected command to trigger a time delay, enabling you to confirm that the command was executed based on the time the application takes to respond. The `ping` command is a good way to do this, because it lets you specify the number of ICMP packets to send. This enables you to control the time taken for the command to run:
...
> [!example] Lab
> [Blind OS command injection with time delays](https://portswigger.net/web-security/os-command-injection/lab-blind-time-delays) — notes at [[Blind OS command injection with time delays]].
...
> [!example] Lab
> [Blind OS command injection with output redirection](https://portswigger.net/web-security/os-command-injection/lab-blind-output-redirection) — notes at [[Blind OS command injection with output redirection]].
...
This payload uses the `nslookup` command to cause a DNS lookup for the specified domain. The attacker can monitor to see if the lookup happens, to confirm whether the command was successfully injected.
